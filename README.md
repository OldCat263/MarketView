...
- 🤖 **智能预测**：缠论 + 10 因子 + 回测 + AI 分析（V2.2.7）
- 🆓 **零成本**：仅用免费公开 API（腾讯/Binance/AkShare/新浪）
- 💾 **磁盘缓存**：V2.0.2 启动恢复 + 30s dump（spot_cache.json + kline_cache.json + watchlist.json）
- 📱 **响应式**：手机/平板/桌面自适应
...
| 模块 | 数据源（优先级从左到右） |
|------|------------------------|
| A股 | **东财 push2（V2.2.0 主源）** → 腾讯 qt.gtimg.cn → 新浪 |
| ETF | **东财 push2（V2.2.0 主源）** → 腾讯 → 同花顺 |
| 港股 | 腾讯 → 东财 → 新浪 |
| 美股 | 腾讯 qt.gtimg.cn（us 前缀，3 分片 V2.1.0）→ 东财 → 新浪 |
| 指数 | **东财 push2（V2.2.0 主源）** → 新浪 |
| 加密 | Binance API（需 `CRYPTO_PROXY`）|
| 新闻 | 新浪财经 → 财新头条（V1.8.0+）|
| K线 | 腾讯 K线 + Binance klines（V1.7.0+）|
| 智能预测 | 缠论 + 回测 + 10 因子 + AI 分析（V1.9.0+，只做 stock+etf）|
...
**版本**：V2.2.7 生产错误修复（美股降级+SSE抑制+卡片就绪同步+重试退避+加密说明）/ V2.2.6 P1 集中修复 11 项 + 仓库瘦身 / V2.2.0 数据源分离 + 预测加速 / V2.0.2 磁盘缓存 / V1.9.0 智能预测系统（缠论+10因子+AI） / V1.8.6 秒开体验（健康检查+首屏快照）
