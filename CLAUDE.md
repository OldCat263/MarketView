...
| 8 | 📰 新闻 | 新浪财经 HTTP（主源）→ 财新头条（fallback）（V1.8.0，60s 刷新，卡片流渲染，点击跳转原文） | 全量实时 | ✅ |
...
- 返回 datetime（发布时间）+ content（内容）+ source（实际媒体来源名称，如"环球市场播报"；财新源为"财新头条"）+ url（原文链接）
- 60s 低频刷新防限流，异常返回空数组不崩主流程
- 前端卡片流渲染，时间倒序，搜索过滤，点击跳转原文（新标签页打开）
- 时间显示：今日 HH:MM:SS，非今日 YYYY-MM-DD HH:MM:SS
...
- **新闻视图**（V1.8.0）：卡片流布局 + 时间倒序 + 搜索过滤 + 点击跳转原文
...
- **响应式适配**（V1.8.0）：平板（≤768px）3 列网格 + K线图例隐藏 + viewTime 隐藏；手机（≤480px）2 列网格 + 面板精简 + 表格横向滚动 + K线工具栏压缩 + 图例隐藏
...
### 3.5 新闻专项检查（V1.8.0+）
- [ ] `/api/news/spot` data 数组非空，datetime/content/source/url 字段完整
- [ ] `/api/stream/news` SSE 连接正常（60s 间隔推送 + 3s 心跳）
- [ ] 卡片流渲染正常（时间倒序、搜索过滤、点击跳转原文）
- [ ] 时间显示：今日 HH:MM:SS，非今日 YYYY-MM-DD HH:MM:SS
- [ ] 新闻面板与表格面板 DOM 不串台
...
| V1.8.0 | 2026-06-27 | **新闻模块**（模块 8 🚧→✅）：新增 `backend/fetcher/news.py`（新浪财经 HTTP 主源 + 财新头条 fallback，60s 刷新，4 字段 datetime/content/source/url），`frontend/js/modules/news.js` 从 placeholder 改为真实模块（卡片流渲染，点击跳转原文，时间显示今日 HH:MM:SS / 非今日 YYYY-MM-DD HH:MM:SS），`core.js` 5 处适配（render 分支 / openModule 互斥 / preloadAll 去 skip / doFilter 分支 / SSE 全量替换 `renderMode==='news'`），`main.py` 加 `/api/news/spot` REST + `/api/stream/news` SSE，`main.css` 加新闻卡片样式（`<a>` 标签去下划线+hover），`index.html` 加 `#newsPanel` 容器。9 文件 +144/-16。铁律全合规。设计稿修订：原 AKShare `js_news` → 金十数据方案已失效（函数移除），改为新浪财经+财新头条。已部署至服务器验证通过。**V1.8.0 收尾**：`main.css` 全面响应式改造（2 断点 768/480px），平板 3 列网格+K线图例隐藏，手机 2 列网格+面板精简+表格横向滚动+K线工具栏压缩+新闻卡片紧凑化，全界面适配手机/平板/桌面。已部署至服务器 |
