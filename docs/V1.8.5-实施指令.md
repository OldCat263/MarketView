...
**改法**：在 `_fetch_tencent` 和 `_fetch_tencent_minute` 函数内使用 `with httpx.Client(...) as client:` 替代裸 `httpx.get()`。**不用模块级 Client**（kline.py 被多线程并发调用，`httpx.Client` 不是线程安全的）。

```python
# _fetch_tencent 函数内（行 30 附近）：
# 原: resp = httpx.get(url, timeout=30)
# 改:
with httpx.Client(timeout=30) as client:
    resp = client.get(url)

# _fetch_tencent_minute 函数内（行 79 附近）同理。
```

> **为什么不用模块级变量**：`main.py` 的 roller 线程和 SSE handler 可能同时调 `_fetch_tencent`，模块级 `httpx.Client` 会在多线程下出现竞态。`with` 语句每次创建新 Client 但复用底层连接池，既安全又比裸 `httpx.get()` 快（省 TLS 握手）。
...
# _heartbeat 也改为广播模式（同步 _sse_queues 结构变化）：
...
> **关键**：`_sse_queues` 从 `{m: Queue()}` 改为 `{m: []}` 后，`_roller`、`stream_module` 和 `_heartbeat` 三个地方都必须同步更新。`_heartbeat` 原代码 `_sse_queues[key].put_nowait(...)` 对 list 调用会立即抛 `AttributeError`。
...
```javascript
// core.js SSE onmessage 的 changed 分支（完整逻辑）

// ① diff 循环前：保存旧值快照（st.rows 即将被 in-place 改写）
let oldSnap = {};
shardRows.forEach(r => {
    let code = r['代码'] || r['交易对'];
    if (!code) return;
...
// ② 执行现有 diff 循环（st.rows 原地更新 old[k] = nv）—— 不变，此处省略
...
if (changed) {
    ...
    if (tab === m) {
        ...
        // ④ 再对新 DOM 加 flash（用 oldSnap 取真旧值，不用 st.rows）
        shardRows.forEach(r => {
            let code = r['代码'] || r['交易对'];
            if (!code) return;
...
> **为什么用 oldSnap 而非 st.rows**：diff 循环已执行 `old[k] = nv`（in-place 更新），st.rows 里的值已是新值。从 st.rows 读"旧值" === 新值 → `nv > ov` 永远 false → 始终闪绿。oldSnap 在 diff 前保存旧值快照，是唯一真实的参考值。
