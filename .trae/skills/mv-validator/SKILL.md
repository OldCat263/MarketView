...
| 命令 | 作用 |
|------|------|
| `python .trae/skills/mv-validator/scripts/mv_validate.py all` | 8 模块数据量 + SSE 心跳 + K线接口 + 铁律自检 + 新闻检查 + 预测端点 |
| `python .trae/skills/mv-validator/scripts/mv_validate.py modules` | 8 模块数据量（6 业务模块 + crypto 单独计 + 新闻）|
| `python .trae/skills/mv-validator/scripts/mv_validate.py sse` | 8 模块 SSE 心跳测试（6s 窗口，应收 1~2 个 shard:-1）|
...
- 8 模块默认顺序：stock / etf / hk / us / index / crypto / news / predict
